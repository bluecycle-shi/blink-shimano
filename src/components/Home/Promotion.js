import React, { createElement } from 'react';
import ProductCard from '../Product/ProductCard';
import { arrOrigin } from '../../constants';

const Promotion = ({ destaque }) => {
    return (
        <>
        <div className="box box__background--grey">
            <div className="container">
                <div className="row">
                    <div className="col-xs-12">
                        <div className="">
                            <h4 className="titulo h4">Ofertas do Mês</h4>
                            {/* <a href="/destaque/promocao" className="link-texto text-decoration-none mr-md-3">ver todo</a> */}
                            {createElement('a', { href: "/destaque/promocao", className: "link-texto text-decoration-none mr-md-3" }, "ver todo")}
                        </div>

                        <div className="cards cards-produto cards-produto-width-4">
                            {destaque.map(product =>
                                <ProductCard
                                    key={`promotion-card-${product.idProduto}`}
                                    product={product}
                                    origin={arrOrigin['home_promocao']}
                                />
                            )}
                        </div>
                    </div>
                </div> 
            </div>
        </div>
        </>
    );
}

export default Promotion;